...
public interface AuthMapper {
	...
	@Insert("INSERT INTO tb_project (`creator_id`, `type`, `name`,`number`, `url`, `desc`, `language`,`state`) "
			+ "VALUES ( #{creator_id}, #{type}, #{name},#{number}, #{url}, #{desc}, #{language}, #{state})")
	...
	// tb_project_user
	@Select("SELECT user_id FROM tb_project_user "
			+ "where project_id = #{project_id} order by flag desc, gmt_create desc")
	...
	int deleteProjectUser(@Param("user_id") String user_id, @Param("project_id") String project_id);
	...
	@Select("SELECT id, plan_id, project_id, creator_id, `state`,issue_count,file_count,"
			+ "extend_issue, resolution_rate, star_time, finish_time, gmt_create, gmt_modified FROM tb_task where id = #{id}")
	...
	@Select("SELECT `id`, `project_id`, `creator_id`, `plan_id`, `state`, `star_time`, `finish_time`, `issue_count`,"
			...
			+ "where project_id=#{project_id} order by id desc limit 0,1")
	...
	@Select("select substring_index((@i:=@i+1),'.',1) as scan_index,t1.issue_count,extend_issue as resolution_count,ifnull(t1.resolution_rate,'0.00') as "
			+ "resolution_rate,t1.id as task_id from tb_task t1,(select @i:=0) t2 where project_id =#{project_id} and `state`='2' order by id;")
	List<Map<String, Object>> selectTaskIndexByProjectId(@Param("project_id") String project_id);

	@Select("select substring_index((@i:=@i+1),'.',1) as scan_index,grade from tb_task t1,(select @i:=0) t2 where project_id = #{project_id} and `state`='2' order by id")
	...
	@Select("SELECT t1.task_id,t3.name as project_name,t3.number as project_number,t4.number as plan_number,t3.`language` as project_language,json_extract(t1.info,'$.type') as type,"
			...
			+ "json_extract(t1.info,'$.severity') as seve,json_extract(t1.info,'$.engine_name') as engine_name FROM tb_report t1,tb_task t2,tb_project t3,tb_plan t4 where t1.task_id = #{task_id} "
			+ "and t2.id = t1.task_id and t3.id = t2.project_id and t4.id = t2.plan_id order by location, begin_line, end_line limit #{current}, #{pageSize}")
	...
	@Select("select issue_count as scanCount from tb_task where project_id = #{project_id} order by id;")
	List<String> selectTaskList(@Param("project_id") String project_id);
	...
	@Select("SELECT json_extract(t1.info,'$.type') as type,json_extract(t1.info,'$.check_name') as check_name,json_extract(t1.info,'$.description') as description,json_extract(t1.info,'$.categories[0]') as categories, "
			...
			+ "case when json_extract(t1.info,'$.location.positions.end.line') is null then json_extract(t1.info,'$.location.lines.end') else json_extract(t1.info,'$.location.positions.end.line') end as end_line, "
			+ "json_extract(t1.info,'$.location.positions.begin.column') as begin_column, json_extract(t1.info,'$.location.positions.end.column') as end_column, json_extract(t1.info,'$.content.body') as body, "
			+ "json_extract(t1.info,'$.severity') as seve,json_extract(t1.info,'$.engine_name') as engine_name FROM tb_report t1 where t1.task_id = #{task_id} order by location, begin_line, end_line ")
