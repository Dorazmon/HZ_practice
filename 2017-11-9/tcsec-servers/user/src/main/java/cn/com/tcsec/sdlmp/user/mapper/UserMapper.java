...
public interface UserMapper {
	...
	@Update("UPDATE tb_user SET name =( CASE WHEN #{name} IS NOT NULL AND #{name} != '' THEN #{name} ELSE `name` END ), "
			+ "    email =( CASE WHEN #{email} IS NOT NULL AND #{email} != '' THEN #{email} ELSE `email` END )  WHERE user_id = #{user_id}")
	int updateUserByUserId(User user) throws Exception;

	@Update("UPDATE tb_user SET company_id = #{company_id} WHERE user_id = #{user_id}")
	int updateUserCompanyIDByUserId(@Param("user_id") String user_id, @Param("company_id") String company_id)
			throws Exception;
	...
	@Update("UPDATE tb_user SET phone=#{phone} WHERE user_id = #{user_id}")
	int updateUserPhoneByUserId(@Param("user_id") String user_id, @Param("phone") String phone) throws Exception;
	...
	@Select("SELECT name, url, `desc`, `state` FROM tb_company where id = #{id}")
	Map<String, String> selectCompanyById(@Param("id") String id) throws Exception;
