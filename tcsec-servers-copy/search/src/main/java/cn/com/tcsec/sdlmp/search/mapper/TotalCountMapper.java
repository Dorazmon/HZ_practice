...
public interface TotalCountMapper {
	...
	@Insert("INSERT INTO `tb_open_total` (`id`, `file_count`, `issue_count`, `line_count`) "
			+ "VALUES ('1',#{file_count}, #{issue_count}, #{line_count})")
	int insertOpenTotal(OpenTotal openTotal) throws Exception;

	@Update("UPDATE `tb_open_total` SET `file_count` = #{file_count}, `issue_count` = #{issue_count},"
