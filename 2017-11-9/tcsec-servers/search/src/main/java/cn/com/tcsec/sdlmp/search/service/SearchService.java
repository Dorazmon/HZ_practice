...
public class SearchService {
	...
	public OpenProject getRandomProject(int index) {
		try {
			return openProjectMapper.selectProjectByIndex(index);
		} catch (Exception e) {
			logger.error("openProjectMapper.selectProjectByIndex 出错", e);
			return null;
		}
	}

	public Object getDangerCoefficient() {
		OpenTotal openTotal = null;
		try {
			openTotal = totalCountMapper.selectOpenTotal();
		} catch (Exception e) {
			logger.error("totalCountMapper.selectOpenTotal 出错", e);
			return ErrorContent.SERVER_ERROR;
		}

		if (openTotal == null) {
			logger.error("totalCountMapper.selectOpenTotal 未找到数据");
			return ErrorContent.SERVER_ERROR;
		}

		Map<String, String> map = new HashMap<>();

		int total = openTotal.getHigh_issue_count() + openTotal.getMid_issue_count() + openTotal.getLow_issue_count();
		int result = (openTotal.getHigh_issue_count() * 55 + openTotal.getMid_issue_count() * 30
				+ openTotal.getLow_issue_count() * 15) / total;

		map.put("value", String.valueOf(result));
