...
public class AuthServerAPIImpl implements AuthServerAPI {
	...
	// project_user write
	@Override
	public Object setProjectUser(String access_token, String project_id, String[] user_id) {
		String userId = access_token.split(":")[0];
		if (!userId.equals(user_id[0]) || !authServer.getAuthProjectAndUserWrite(userId, project_id)) {
			return ErrorContent.SYSTEM_REJECT;
		}
		return authServer.setProjectUser(project_id, user_id);
	}
