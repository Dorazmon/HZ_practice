...
public class AuthService {
	...
	public Object addProject(String user_id, String type, String name, String number, String url, String desc,
		...
		int project_count = authMapper.selectProjectCountByUserId(user_id);
	...
	public Object setProjectGroup(String project_id, String[] user_list) {
		List<String> projectGroup = null;
		try {
			projectGroup = authMapper.selectAllProjectUserByProjectId(project_id);
		} catch (Exception e) {
			logger.error("Exception	", e);
			return ErrorContent.SERVER_ERROR;
		}
		...
		String projectAdmin = user_list[0];

		for (String user_id : user_list) {
			if (!projectGroup.contains(user_id)) {
				needToAddUser.add(user_id);
			}
		}

		for (String user_id : projectGroup) {
			boolean flag = false;
		...
		if (needToDelUser.size() != 0) {
			System.out.println(needToDelUser.toArray(new String[needToDelUser.size()])[0]);
		}
	...
	public Object setProjectUser(String project_id, String[] user_list) {
	...
	private Object addProjectUser(String project_id, String[] user_id, String projectAdmin) {
		...
		for (String userId : user_id) {
			if (userId.equals(projectAdmin)) {
				...
				map.put("contact", userId);
				try {
					response = restClient.send("getUser", map, Object.class);
				} catch (Throwable e) {
					logger.error("restClient	getUser error!", e);
					return ErrorContent.SERVER_ERROR;
				}

				if (response != null || !(response instanceof String) || !userId.equals(response)) {
					logger.error("restClient	getUser not found user :{}", userId);
					return ErrorContent.SERVER_ERROR;
				}
	...
	public Object addPlan(String project_id, String user_id, String type, String period_flag, String timeParam,
		...
		int count = authMapper.selectPlanCountByProjectId(project_id);
		if (count >= 2) {
			return ErrorContent.SYSTEM_REJECT;
		}
	...
	public Object getOutline(String project_id) {
		...
		List<Map<String, Object>> list = authMapper.selectTaskList(project_id);
		if (list == null) {
			logger.error("authMapper.selectTaskList error!");
			return ErrorContent.SERVER_ERROR;
		}
		...
		if (list.isEmpty()) {
			...
			map1.put("count", list.get(0).get("issue_count"));
			map2.put("count", list.get(0).get("file_count"));
			map3.put("count", list.size());
			map4.put("count", list.get(0).get("resolution_rate"));
			if (list.size() == 1) {
				map1.put("desc", desclUp + "100%");
				map1.put("level", levelUp);
				map2.put("desc", desclUp + "100%");
				map2.put("level", levelUp);
				map3.put("desc", "扫描时间" + desclUp + "100%");
				map3.put("level", levelUp);
				map4.put("desc", desclUp + "100%");
				map4.put("level", levelUp);
			} else {
				long thisCount = (long) list.get(0).get("issue_count");
				long lastCount = (long) list.get(1).get("issue_count");
				if (thisCount < lastCount) {
					map1.put("desc", descDown + df.format(((float) (lastCount - thisCount)) / (float) lastCount) + "%");
					map1.put("level", levelDown);
				} else if (thisCount == lastCount) {
					map1.put("desc", descEquality);
					map1.put("level", levelEquality);
				} else {
					map1.put("desc", desclUp + ((lastCount == 0) ? "100"
							: df.format(((float) (thisCount - lastCount)) / (float) lastCount)) + "%");
					map1.put("level", levelUp);
				}

				thisCount = (long) list.get(0).get("file_count");
				lastCount = (long) list.get(1).get("file_count");
				if (thisCount < lastCount) {
					map2.put("desc", descDown + df.format(((long) (lastCount - thisCount)) / (long) lastCount) + "%");
					map2.put("level", levelDown);
				} else if (thisCount == lastCount) {
					map2.put("desc", descEquality);
					map2.put("level", levelEquality);
				} else {
					map2.put("desc", desclUp + ((lastCount == 0) ? "100"
							: df.format(((long) (thisCount - lastCount)) / (long) lastCount)) + "%");
					map2.put("level", levelUp);
				}

				thisCount = (long) list.get(0).get("scan_time");
				lastCount = (long) list.get(1).get("scan_time");
				if (thisCount < lastCount) {
					map3.put("desc",
							"扫描时间" + descDown + df.format(((long) (lastCount - thisCount)) / (long) lastCount) + "%");
					map3.put("level", levelDown);
				} else if (thisCount == lastCount) {
					map3.put("desc", "扫描时间" + descEquality);
					map3.put("level", levelEquality);
				} else {
					map3.put("desc", "扫描时间" + desclUp + ((lastCount == 0) ? "100"
							: df.format(((long) (thisCount - lastCount)) / (long) lastCount)) + "%");
					map3.put("level", levelUp);
				}

				thisCount = (long) Float.parseFloat(list.get(0).get("resolution_rate").toString());
				lastCount = (long) Float.parseFloat(list.get(1).get("resolution_rate").toString());
				if (thisCount < lastCount) {
					map4.put("desc", descDown + df.format(((long) (lastCount - thisCount)) / (long) lastCount) + "%");
					map4.put("level", levelDown);
				} else if (thisCount == lastCount) {
					map4.put("desc", descEquality);
					map4.put("level", levelEquality);
				} else {
					map4.put("desc", desclUp + ((lastCount == 0) ? "100"
							: df.format(((long) (thisCount - lastCount)) / (long) lastCount)) + "%");
					map4.put("level", levelUp);
				}
			}
	...
	@SuppressWarnings("unchecked")
	public Object getChartBugInfoByTaskId(String task_id, String risk_type) {
		List<IssueLevel> issueList = null;
		try {
			issueList = issueLevelMapper.selectIssueLevelList();
		} catch (Exception e) {
		...
		Task task = null;
		try {
			task = authMapper.selectTask(task_id);
		} catch (Exception e) {
			logger.error("Select LastTask By ProjectId 出错", e);
			return ErrorContent.SERVER_ERROR;
		}
